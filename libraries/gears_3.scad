...
module spur_gear(modul, tooth_number, width, bore, pressure_angle = 20, helix_angle = 0, optimized = true, correction=0) {
    ...
    echo("D = ", d);
    echo("DF = ", df);
    echo("DF with correction = ", df-correction);
    
    pp = [for (rho = [0:step:rho_ra]) polar_to_cartesian(ev(rb,rho))];
    ...
    union(){
            ...
            linear_extrude(height = width, convexity = 10, twist = gamma){
                difference(){
                    union(){
                        ...
                        circle(rf-correction/2);                                     // Root Circle
                        for (rot = [0:tau:360]){
                    ...
                    circle(r = rm+r_hole*1.49-correction/2);                         // "bore"
                }
            }
        }
        ...
        if (optimized) {
            ...
            linear_extrude(height = (width-r_hole/2 < width*2/3) ? width*2/3 : width-r_hole/2, convexity = 10){
                difference(){
                    circle(r=rm+r_hole*1.51-correction/2);
                    union(){
                        circle(r=(bore+r_hole)/2);
                        for (i = [0:1:z_hole]){
