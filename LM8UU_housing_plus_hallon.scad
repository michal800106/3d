...
rotate ([0,0,-90])#translate ([0,-5.5,15-3]) rotate ([0,90,0]) cylinder (d=1.2, h = 49,center = true); // insert the pin
 
 if (bottom == "no") rotate ([0,0,-90])#translate ([0,-5.5,-15+2.5]) rotate ([0,90,0]) cylinder (d=1.2, h = 49,center = true); // insert the bottom pin
...
difference() {
    intersection() {
        color("yellow")
        import(
"/home/michalz/Downloads/HallON_Anet_A8_ABL_Sensor/files/Hallon_Bottom_plate_4mm_rod.stl"
        );
        translate([4, 0, 0])
        cube([73, 73, 73], center=true);
    };
    #translate([-3.7, 0, 16])
    rotate([90,-90,0])
    union() {
        translate ([0,0,2])
        cylinder (d=bushing_diam, h = 29,center = true) ;

        rotate ([0,0,-90])
        #translate ([0,-5.5,15-3])
        rotate ([0,90,0])
        cylinder (d=1.2, h = 49,center = true); // insert the pin

        if (bottom == "no")
            rotate ([0,0,-90])
            #translate ([0,-5.5,-15+2.5])
            rotate ([0,90,0])
            cylinder (d=1.2, h = 49,center = true); // insert the bottom pin
    };
};
